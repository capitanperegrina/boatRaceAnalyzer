...
public class DefaultBoatRaceAnalyzerController {
	...
	public ResponseEntity<String> generateTrackJavascriptFile(@RequestParam(name = "r", required = true) Integer raceId,
			...
			@RequestParam(name = "b", required = true) Integer boatId, Model model) {
		HttpHeaders responseHeaders = new HttpHeaders();
		responseHeaders.setContentType(MediaType.TEXT_PLAIN);
		return new ResponseEntity<>(this.navionicsHtmlGeneratorService.generateJavascriptVariableTrack(raceId, legId, boatId),
